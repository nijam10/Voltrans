<div class="bg-gradient-to-t from-[#4C956C] to-[#2C6E6D] text-white px-6 py-10">
    <div class="max-w-7xl mx-auto flex flex-col md:flex-row gap-10">
        <!-- Logo dan Navigasi -->
        <div class="flex-1">
            <div class="flex items-center mb-4">
                <img src="images/voltrans.png" alt="Logo" class="w-16 h-16 md:w-20 md:h-20">
                <span class="font-bold text-lg mt-3">Voltrans</span>
            </div>

            <h3 class="font-bold text-lg mb-4">Navigasi</h3>
            <ul class="grid grid-cols-2 gap-y-2 mb-4 text-sm">
                <li><a href="#" class="hover:text-slate-200 transition-colors">Beranda</a></li>
                <li><a href="#" class="hover:text-slate-200 transition-colors">Tentang</a></li>
                <li><a href="#" class="hover:text-slate-200 transition-colors">Produk</a></li>
                <li><a href="#" class="hover:text-slate-200 transition-colors">Ulasan</a></li>
                <li><a href="#" class="hover:text-slate-200 transition-colors">Hubungi Kami</a></li>
                <li><a href="#" class="hover:text-slate-200 transition-colors">Sewa</a></li>
            </ul>

            <!-- Ikon Sosial Media -->
            <div class="flex space-x-4 text-xl">
                <a href="#" class="hover:text-slate-200 transition-colors"><i class="fa-brands fa-facebook-f"></i></a>
                <a href="#" class="hover:text-slate-200 transition-colors"><i class="fa-brands fa-instagram"></i></a>
                <a href="#" class="hover:text-slate-200 transition-colors"><i class="fa-brands fa-linkedin-in"></i></a>
                <a href="#" class="hover:text-slate-200 transition-colors"><i class="fa-brands fa-twitter"></i></a>
            </div>
        </div>

        <!-- Kontak -->
        <div class="flex-1 lg:mt-24">
            <h3 class="font-bold text-lg mb-4">Hubungi Kami</h3>
            <div class="flex items-start mb-3">
                <i class="fa-solid fa-phone mt-1 mr-3"></i>
                <span>+62 7789012</span>
            </div>
            <div class="flex items-start mb-3">
                <i class="fa-solid fa-envelope mt-1 mr-3"></i>
                <span>contact@voltrans.com</span>
            </div>
            <div class="flex items-start">
                <i class="fa-solid fa-location-dot mt-1 mr-3"></i>
                <div class="text-sm">
                    <p>Politeknik Negeri Batam, Jalan Ahmad Yani,</p>
                    <p>Kelurahan Teluk Tering, Kecamatan Batam Kota,</p>
                    <p>Kota Batam, Provinsi Kepulauan Riau,</p>
                    <p>Kode Pos 29461, Indonesia.</p>
                </div>
            </div>
        </div>

        <!-- Jam Operasional -->
        <div class="flex-1">
            <h3 class="font-bold text-lg lg:mt-23">Temui Kami</h3>
            <p class="text-sm mt-6">
                Jika ada kendala atau pertanyaan, silahkan datang ke lokasi kantor kami pada jam operasional berikut.
            </p>
            <div class="flex flex-col text-sm space-y-2 mt-4">
                <div class="flex items-center">
                    <i class="fa-regular fa-clock mr-3"></i>
                    <span>Senin - Jumat: 08:00 - 17:00</span>
                </div>
                <div class="flex items-center">
                    <i class="fa-regular fa-clock mr-3 opacity-0"></i>
                    <span>Sabtu: 08:00 - 14:00</span>
                </div>
                <div class="flex items-center">
                    <i class="fa-regular fa-clock mr-3 opacity-0"></i>
                    <span>Minggu: Tutup</span>
                </div>
            </div>
        </div>
    </div>

    <aside class="text-center mt-10 text-sm">
        <p>Copyright © 2025 - All right reserved by Voltrans Rent Company 🚗🚘</p>
    </aside>
</div>
